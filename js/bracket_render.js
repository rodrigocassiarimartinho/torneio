    ...
    for (let r = 0; r < rounds.length - 1; r++) {
        ...
        if (nextRoundMatches.length === currentRoundMatches.length / 2) {
            currentRoundMatches.forEach((match, index) => {
                ...
                if (nextMatch) createPath(match, nextMatch, svg, wrapper);
            });
        }
    }
}

function createPath(startEl, endEl, svg, wrapper) {
    const wrapperRect = wrapper.getBoundingClientRect();
    const startRect = startEl.getBoundingClientRect();
    const endRect = endEl.getBoundingClientRect();
    if (startRect.width === 0 || endRect.width === 0) return;
    const startX = startRect.right - wrapperRect.left;
    const startY = startRect.top + startRect.height / 2 - wrapperRect.top;
    const endX = endRect.left - wrapperRect.left;
    const endY = endRect.top + endRect.height / 2 - wrapperRect.top;
    const midX = startX + (CONFIG.ROUND_GAP / 2);
